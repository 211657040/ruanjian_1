...
<div class="main">
    ...
    <h2>
        ...
        <a href="_superadblog.php"><span class="curr">博客管理</span></a>
        <a href="_blogcenter.php">博客中心</a>
    </h2>
    <?php
    include_once 'conn.php';
    include_once 'page.php';
    $sql = "select count(id) as total from blog_posts";
    ...
    $sql = "select * from blog_posts order by id asc limit $firstCount,$perPage";//limit后面两参数传递从哪一个开始取,一次取多少条
    $result = mysqli_query($conn, $sql);
    if (!$result) {
...
<script>
    function del(id,name){
        if(confirm('确定删除博客:' + name + '?')){
            location.href='_Superdelblog.php?id=' + id + '&title=' + name;
        }
    }
    function xiugai_(id,name){
        if(confirm('确定修改博客:' + name + '?')){
            location.href='_Supermodifyblog.php?id=' + id + '&title=' + name;
        }
    }
</script>
</body>
